...
router.all('/api/postback/perfectpay', async (req, res) => {
    ...
    try {
        ...
        // Identify product type (front/upsell1-7)
        // PerfectPay English: PPPBEGBG=Front, PPPBDG0I=UP1, PPPBDG0J=UP2, PPPBEIDH=UP3, PPPBEIDL=UP4, PPPBEIDM=UP5, PPPBEIDN=UP6, PPPBEIDP=UP7
        let productType = 'front';
        const ppCodeStr = String(productCode || '').toUpperCase();
        if (ppCodeStr === 'PPPBDG0I' || productNameLower.includes('message vault') || productNameLower.includes('recover')) {
            productType = 'upsell1';
        } else if (ppCodeStr === 'PPPBDG0J' || productNameLower.includes('360') || productNameLower.includes('tracker') || productNameLower.includes('social')) {
            productType = 'upsell2';
        } else if (ppCodeStr === 'PPPBEIDH' || productNameLower.includes('instant') || productNameLower.includes('vip') || productNameLower.includes('priority')) {
            productType = 'upsell3';
        } else if (ppCodeStr === 'PPPBEIDL' || productNameLower.includes('invisibility') || productNameLower.includes('cloak') || productNameLower.includes('manto invisible')) {
            productType = 'upsell4';
        } else if (ppCodeStr === 'PPPBEIDM' || productNameLower.includes('live room') || productNameLower.includes('live access') || productNameLower.includes('acceso en vivo')) {
            productType = 'upsell5';
        } else if (ppCodeStr === 'PPPBEIDN' || productNameLower.includes('multi-device') || productNameLower.includes('multi-dispositivo')) {
            productType = 'upsell6';
        } else if (ppCodeStr === 'PPPBEIDP' || productNameLower.includes('behavior analyst') || productNameLower.includes('analista de comportamiento')) {
            productType = 'upsell7';
